...
function checkData(frm) {
    ...
    var GoodsName = frm.GoodsName.value;
    // 상점아이디 검증
    ...
    //상품명 검증
    if (GoodsName == undefined || GoodsName.trim() == '' || GoodsName == null) {
        alert("상품명을 입력해 주세요.");
        return false;
    }
...
function makeEncKey(frm) {
    ...
    try {
        if (frm.DutyFreeAmt.value != undefined && frm.DutyFreeAmt.value != null && frm.DutyFreeAmt.value.trim() != '') {
            if (!isNaN(parseInt(frm.DutyFreeAmt.value))) {
                DutyFreeAmt = parseInt(frm.DutyFreeAmt.value);
            }
